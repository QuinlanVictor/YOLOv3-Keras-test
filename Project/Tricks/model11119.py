...
20201103
因为要再准备一篇文章，也为了后面的大论文做准备，再测试一种网络结构
修改一下网络结构，采用双尺度输出，选择4倍和8倍下采样的特征图进行检测

1119  对于ECA注意力模块进行测试，在model1111_2的基础上进行修改
      思考一下eca模块的实现原理
      结合学习的项目思考一下keras的编程
...
from keras.layers import Conv2D, Add, ZeroPadding2D, UpSampling2D, Concatenate, MaxPooling2D, GlobalAveragePooling2D, Reshape,\
    Dense, multiply,Activation, Input, Dropout, Conv1D
...
def yolo_body(inputs, num_anchors, num_classes):
    ...
    feat3=eca_layer(feat3)
    x = make_five_convs(feat3, 512)

    x = compose(
            DarknetConv2D_BN_Leaky(256, (1,1)),
            UpSampling2D(2))(x)
    feat2=eca_layer(feat2)
...
def preprocess_true_boxes(true_boxes, input_shape, anchors, num_classes):
    ...
    grid_shapes = [input_shape//{0:8, 1:4}[l] for l in range(num_layers)]
