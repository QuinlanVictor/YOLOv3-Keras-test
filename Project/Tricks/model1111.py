...
note 1111 在model1103的基础上，继续修改网络结构，加入CBAM注意力机制模块
          使用train1111.py测试了一下，能够成功训练
"""
...
from keras.layers import Conv2D, Add, ZeroPadding2D, UpSampling2D, Concatenate, MaxPooling2D, GlobalAveragePooling2D,\
    GlobalMaxPooling2D, Reshape, Dense, multiply, Permute, Activation, Lambda
...
def yolo_body(inputs, num_anchors, num_classes):
    ...
    feat3=cbam_block(feat3)
    x = make_five_convs(feat3, 512)

    x = compose(
            DarknetConv2D_BN_Leaky(256, (1,1)),
            UpSampling2D(2))(x)
    feat2=cbam_block(feat2)
